...
AmritaCore 提供了一套类型化的数据容器，构成对话状态、消息传递和上下文管理的基础。这些容器定义在 `amrita_core.types` 包中，并与[数据后端](data-backend.md)集成以实现持久化。
...
`ToolResult` 可以和 `Message` 对象一起出现在 `memory.messages` 中——两者都是有效的 `CONTENT_LIST_TYPE_ITEM`。
...
- `memory`：`MemoryModel` — 对话记忆
- `ability`：`AbilityContext` — 工具、预设、MCP 客户端
...
ability = AbilityContext(
    tools=ToolsManager(),       # 默认使用全局 ToolsManager 单例
    presets=PresetManager(),    # 默认使用全局 PresetManager 单例
    mcp=ClientManager(),        # 默认使用全局 ClientManager 单例
    extra={}
)
```
...
AmritaCore 为管理器类使用两层模式：**`Multi*Manager`** 基类拥有实例级状态，**单例子类**提供全局共享实例。单例是 `AbilityContext` 和 `LegacyBackend` 使用的默认值。

### ToolsManager / MultiToolsManager

[`MultiToolsManager`](../api-reference/classes/MultiToolsManager.md) 是一个工具注册表，按函数名存储 [`ToolData`](../api-reference/classes/ToolData.md) 条目。每个 `ToolData` 将 schema（`ToolFunctionSchema`）与异步可调用对象和可选的 `enable_if()` 谓词绑定。
...
| `get_tool(name)`                           | 获取完整 `ToolData`（遵守 `enable_if()`）      |
| `get_tool_meta(name)`                      | 仅获取 `ToolFunctionSchema`                    |
| `get_tool_func(name)`                      | 获取原始异步可调用对象                         |
| `get_tools()`                              | 获取所有启用的工具，返回 `dict[str, ToolData]` |
| `tools_meta()` / `tools_meta_dict()`       | 获取所有工具 schema 用于 LLM 请求              |
| `register_tool(tool)`                      | 注册一个 `ToolData`                            |
| `remove_tool(name)`                        | 移除并取消禁用工具                             |
| `enable_tool(name)` / `disable_tool(name)` | 运行时启用/禁用切换                            |

[`ToolsManager`](../api-reference/classes/ToolsManager.md) 是 `MultiToolsManager` 的**单例**子类——所有使用 `LegacyBackend` 的会话共享同一个全局工具注册表。通过 `@simple_tool` / `@on_tools` 注册的工具会落入此全局实例。

### PresetManager / MultiPresetManager

[`MultiPresetManager`](../api-reference/classes/MultiPresetManager.md) 管理 [`ModelPreset`](data-misc.md#modelpreset-模型预设) 实例集合：
...
| 方法                       | 描述                                         |
| -------------------------- | -------------------------------------------- |
| `add_preset(preset)`       | 注册 `ModelPreset`（重名则抛出异常）         |
| `get_preset(name)`         | 按名称查找（不存在则抛出 `ValueError`）      |
| `set_default_preset(name)` | 设置回退预设                                 |
| `get_default_preset()`     | 返回默认值，若未设置则随机选择一个已注册预设 |
| `get_all_presets()`        | 返回 `list[ModelPreset]`                     |
| `test_presets()`           | 异步生成器，为每个预设产出 `PresetReport`    |

[`PresetManager`](../api-reference/classes/PresetManager.md) 是**单例**子类——所有会话默认共享同一预设目录。

### ClientManager / MultiClientManager（MCP）

[`MultiClientManager`](../api-reference/classes/MultiClientManager.md) 管理 [`MCPClient`](../api-reference/classes/MCPClient.md) 实例池，将外部 MCP 服务器桥接到工具系统：
...
当 MCP 服务器加载时，其工具会自动注册到绑定的 `tools_manager` 中。名称冲突会触发自动重映射（如 `search` → `referred_42_search`）。详见 [MCP 服务器集成](../extensions-integration/mcp-server-integration.md) 的端到端示例。

[`ClientManager`](../api-reference/classes/ClientManager.md) 是**单例**子类。每个 `MCPClient` 包装一个单独的 MCP 服务器脚本，并处理连接生命周期（连接、基于 TTL 的自动关闭、工具发现）。
