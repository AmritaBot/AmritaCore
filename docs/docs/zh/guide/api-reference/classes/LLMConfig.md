...
- `memory_length_limit` (int)：默认 `50`。记忆上下文中的最大消息数
- `max_tokens` (int)：默认 `1000`。单次响应中生成的最大 token 数
- `tokens_count_mode` (Literal["word", "bpe", "char"])：默认 `"bpe"`。token 计数模式
- `enable_tokens_limit` (bool)：默认 `True`。是否启用上下文长度限制
- `session_tokens_windows` (int)：默认 `5000`。会话 token 窗口大小
- `llm_timeout` (int)：默认 `60`。API 请求超时时间（秒）
