...
> 从 AmritaCore v0.9.0rc1 开始，核心基础设施已提取到 `amrita-sense` 包。

## 概述

`amrita-sense` (>=0.2.1) 是 AmritaCore 的**基础运行时**，提供工作流引擎、事件系统和流式处理能力。AmritaCore 在此基础上构建 Agent 层（策略、会话、工具、MCP、适配器）。
...
| `amrita_core.logging`                                             | `amrita_sense.logging`        | 内置 — 无独立文档                                                                          |
...
| `amrita_core.hook.exception`                                      | `amrita_sense.hook.exception` | [流程控制](https://sense.amritabot.com/guide/concepts/flow_control)                        |
| 工作流引擎 (`Node`, `NodeComposeRendered`, `WorkflowInterpreter`) | `amrita_sense` 顶级           | [组合与执行](https://sense.amritabot.com/guide/concepts/compose_and_exec)                  |
| `ALIAS`, `ARCHIVED_NODES`                                         | `amrita_sense` 顶级           | [流程控制](https://sense.amritabot.com/guide/concepts/flow_control)                        |
...
旧导入路径在 v0.10.x+ 中已移除；请直接从 `amrita_sense` 导入。
